...
func (t *Face) BatchVerifyIdcardAndPhone(concurrency, failCount int, data map[string]*IdcardPhoneTuple) (int, int, int) {
	...
	for _, v := range data {
		...
		go func(v *IdcardPhoneTuple) {
			...
			ok, desc, requestId, err := t.VerifyIdcardAndPhone(v.Idcard, v.Name, v.Phone)
			if err != nil {
				v.Err = err
				v.Consistent = false
				v.ErrDesc = ""
...
func (t *Face) VerifyIdcardAndPhone(idcard, name, phone string) (bool, string, string, error) {
