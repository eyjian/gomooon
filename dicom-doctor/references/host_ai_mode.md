...
5. **🚨 模型能力检测可能不准确**：`model_capability_detector.py` 通过环境变量和模型名称推断宿主 AI 是否支持视觉。**如果检测结果与实际不符（例如检测说"不支持视觉"但你实际上可以读取图片），请忽略检测结果，直接按 SKILL.md 流程逐批阅片。** 检测误判不是停止阅片的理由。v2.9.0 已将默认策略改为"乐观"（假定支持视觉），大幅降低误判概率。
6. **宿主 AI 必须传入 `--model-name`**：执行第 1 步时，宿主 AI 应将自身模型名称通过 `--model-name` 参数传入（如 `--model-name claude-4.6-opus`），这能显著提高能力检测的准确性。
