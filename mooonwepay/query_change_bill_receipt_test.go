...
// go test -v -run="TestQueryChangeBillReceipt$" -args private_key.pem mchid serial_no out_batch_no <out_detail_no>
func TestQueryChangeBillReceipt(t *testing.T) {
	numArgs := len(os.Args)
	...
	t.Log(*req)
	resp, err := QueryChangeBillReceipt(req)
	if err != nil {
		t.Error(err)
	}
	if resp != nil {
